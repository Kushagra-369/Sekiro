...
export default function Enemies() {
  ...
  useEffect(() => {
    ...
    const scrollSpeed = () => {
      ...
      if (width < 768) return 1;       // slow on mobile
      if (width < 1024) return 2;      // medium on tablet
      return 3;                        // default on desktop
    };
  ...
  return (
    <div
      ...
      style={{
        background: 'url(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRJlfDFYGT2-kLi9ky9bwIZk8g5UA5c_1zFKg&s)',
    ...
    >
      ...
      <div
        ...
        className="flex overflow-x-auto space-x-4 px-4 pb-4"
        style={{ scrollBehavior: "smooth" }}
      >
        {data.map(enemy => (
          <div key={enemy.id} className="flex-shrink-0 text-center min-w-[300px]">
            <a href={enemy.link} target="_blank" rel="noopener noreferrer">
              <img
                src={enemy.img}
                alt={enemy.name}
                className="h-80 w-80 mx-auto rounded hover:scale-105 transition-all duration-100"
              />
            </a>
